...
export async function GET(request: NextRequest) {
  try {
    ...
    // Buscar posts ordenados por engajamento (likes + comments + shares)
    // Considerando posts das últimas 30 dias para manter relevância
    ...
    const { data: posts, error } = await supabase
      ...
      .limit(limit * 2); // Buscar mais para calcular engajamento
    ...
    // Calcular score de engajamento e ordenar
    ...
    const postsWithScore = (posts as PostWithCounts[] || []).map((post) => {
    ...
    // Ordenar por score e pegar os top posts
